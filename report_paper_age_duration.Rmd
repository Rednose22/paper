...
coeff.mod1.f.org <- do.call("bind_rows", comb.list.durn[[1]]) %>%
  ...
  mutate("Mean (95% CI)" = paste0(mean, "(", lower, ", ", upper, ")"),
         ...
         scenario = factor(scenario, levels = c("Base scenario", "Sub-groups scenario", "No sub-groups scenario"))) %>%
  arrange(parameter, scenario)

## forest figure
study1_durn <- coe.forest.fig(paste(c('durnpso', '^d'),collapse="|"), -3, 5.5, -2, 0.6, coeff.mod1.f.org) +
  labs(title = "Comparison of coefficient of model")
study1_durn
...
## duration of psoriasis
study2_durn <- coe.forest.fig(paste(c('durnpso', '^d'),collapse="|"), -2.5, 5, -1.5, 0.6, coeff.mod1.f.2)+
